...
public class BasePermissionsActivity extends Activity {
    public final static int REQUEST_CODE_GEOLOCATION = 2,
            ...
            REQUEST_CODE_PUSH_NOTIFICATION = 222;
    ...
    protected boolean checkPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == 104) {
            ...
            if (!granted) {
                showPermissionErrorAlert(R.raw.permission_request_folder, requestCode == REQUEST_CODE_EXTERNAL_STORAGE_FOR_AVATAR ? LocaleController.getString("PermissionNoStorageAvatarSword", R.string.PermissionNoStorageAvatarSword) :
                        LocaleController.getString("PermissionStorageWithHintSword", R.string.PermissionStorageWithHintSword));
            } else {
                ImageLoader.getInstance().checkMediaPaths();
            }
        } else if (requestCode == REQUEST_CODE_ATTACH_CONTACT) {
            if (!granted) {
                showPermissionErrorAlert(R.raw.permission_request_contacts, LocaleController.getString("PermissionNoContactsSharingSword", R.string.PermissionNoContactsSharingSword));
                return false;
            } else {
            ...
            if (requestCode == REQUEST_CODE_VIDEO_MESSAGE && (!audioGranted || !cameraGranted)) {
                showPermissionErrorAlert(R.raw.permission_request_camera, LocaleController.getString("PermissionNoCameraMicVideoSword", R.string.PermissionNoCameraMicVideoSword));
            } else if (!audioGranted) {
                showPermissionErrorAlert(R.raw.permission_request_microphone, LocaleController.getString("PermissionNoAudioWithHintSword", R.string.PermissionNoAudioWithHintSword));
            } else if (!cameraGranted) {
                showPermissionErrorAlert(R.raw.permission_request_camera, LocaleController.getString("PermissionNoCameraWithHintSword", R.string.PermissionNoCameraWithHintSword));
            } else {
        ...
        else if (requestCode == REQUEST_CODE_PUSH_NOTIFICATION){
            if (!granted) {
//                showPermissionErrorAlert(R.raw.permission_request_camera, LocaleController.getString("PermissionNoCameraWithHintSword", R.string.PermissionNoCameraWithHintSword));
            }
        }
