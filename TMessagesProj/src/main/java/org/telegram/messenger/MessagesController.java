    ...
    public TLRPC.InputPeer getInputPeer(long id) {
        ...
        if (id < 0) {
            ...
            if (ChatObject.isChannel(chat)) {
                ...
                inputPeer.id = -id;
                inputPeer.type = "inputPeerChannel";
                inputPeer.access_hash = chat.access_hash;
                Log.e("fvbjfbv","inputPeerChannel");
            } else {
                inputPeer = new TLRPC.TL_inputPeerChat();
                inputPeer.type = "inputPeerChat";
                inputPeer.chat_id = -id;
                inputPeer.id = -id;
                Log.e("fvbjfbv","inputPeerChat");
            }
        } else {
            TLRPC.User user = getUser(id);
            inputPeer = new TLRPC.TL_inputPeerUser();
            inputPeer.type = "inputPeerUser";
