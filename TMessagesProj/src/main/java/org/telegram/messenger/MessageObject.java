...
import org.telegram.sword.swordTeleModels.local.AcceptInfo;
...
public class MessageObject {
    ...
    public static final int TYPE_F_ = 333;
    ...
    public boolean isF_Files = false;
...
//        Log.e("dvbjdbfv",message.message.toString());
    }
...
//                Log.e("nbjvbvf",String.valueOf(messageOwner.message));
            ...
            if (!TextUtils.isEmpty(restrictionReason)) {
                ...
                if (getMedia(messageOwner) instanceof TLRPC.TL_messageMediaDice) {
                    ...
                    messageText = LocaleController.getString("UnsupportedMediaSword", R.string.UnsupportedMediaSword);
            ...
            if (isRestrictedMessage) {
                ...
                if (document != null && document.mime_type != null) {
                    if (isGifDocument(document, hasValidGroupId())) {
                        ...
                        if (mime.toLowerCase().startsWith("sword")){
...
//            }
    ...
    public float getTextHeight(String text,int maxWidth){

        final float lineAdd = totalAnimatedEmojiCount >= 4 ? -1 : 0;

        StaticLayout textLayout = new StaticLayout(text, Theme.chat_msgTextPaint, maxWidth, Layout.Alignment.ALIGN_NORMAL, 1f, lineAdd, false);

      return   textHeight = textLayout.getHeight();
    ...
    public boolean isDocument() {
        ...
        boolean F_Item = false;

        if (getDocument()!=null){

            String m =""+getDocument().mime_type;

            F_Item = m.trim().toLowerCase().startsWith("sword");
        }

        return F_Item && getDocument() != null && !isVideo() && !isMusic() && !isVoice() && !isAnyKindOfSticker();
    }
