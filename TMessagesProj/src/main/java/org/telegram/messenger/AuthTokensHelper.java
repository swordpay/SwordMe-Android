...
public class AuthTokensHelper {
    ...
    public static ArrayList<TLRPC.TL_auth_authorization> getSavedLogInTokens() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Log.e("saveLogInToken","1111         "+value);
            SerializedData serializedData = new SerializedData(Utilities.hexToBytes(value));
    ...
    public static String saveLogInToken(TLRPC.TL_auth_authorization token) {

            Log.e("saveLogInToken " ,"222222222         "+ new String(token.future_auth_token, StandardCharsets.UTF_8));
        ...
        tokens.add(0, token);
       return saveLogInTokens(tokens);
    }

    private static String saveLogInTokens(ArrayList<TLRPC.TL_auth_authorization> tokens) {
        String token  = "";
        ...
        if (activeTokens.size() > 0) {
            ...
            for (int i = 0; i < activeTokens.size(); i++) {
                SerializedData data = new SerializedData(activeTokens.get(i).getObjectSize());
                activeTokens.get(i).serializeToStream(data);
                Log.e("saveLogInToken " ,"3333         "+ Utilities.bytesToHex(data.toByteArray()));
                editor.putString("log_in_token_" + i, Utilities.bytesToHex(data.toByteArray()));

                token = Utilities.bytesToHex(data.toByteArray());
            }
            editor.apply();
